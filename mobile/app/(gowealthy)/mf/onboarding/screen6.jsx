...
addr3 = (lines[2] || '').slice(0, 40).toUpperCase();
    ...
    >
      <View style={styles.modalOverlay}>
        <View style={styles.modalSheet}>
          ...
          {/* Open link button */}
          {authLink ? (
            <TouchableOpacity onPress={handleOpenAuthLink} activeOpacity={0.9} style={styles.modalAuthBtnWrap}>
              <LinearGradient
                colors={[C.o2, C.o]}
                start={{ x: 0, y: 0 }}
                end={{ x: 0, y: 1 }}
                style={styles.modalAuthBtn}
              >
                <Text style={styles.modalAuthBtnText}>🔗 Open NSE Authorization Page →</Text>
              </LinearGradient>
            </TouchableOpacity>
  ...
  >
    {checkingAuth ? (
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
        <ActivityIndicator size="small" color="#062018" />
        <Text style={styles.modalDoneBtnText}>Checking…</Text>
      </View>
    ) : (
      <Text style={styles.modalDoneBtnText}>
      ...
      <ScrollView style={styles.container} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.questionSection}>
          <View style={styles.eyebrowRow}>
            ...
            <Text style={styles.eyebrow}>REGULATORY PROFILE</Text>
            <View style={styles.eyebrowLine} />
          </View>
          <Text style={styles.questionTitle}>
            Complete your <Text style={styles.gradWord}>Profile</Text>
          </Text>
          <Text style={styles.questionSubtitle}>
            Last step — a few details required for regulatory compliance
          </Text>
        ...
        <View style={styles.buttonSection}>
          ...
          {isSubmitting && (
            <View style={styles.progressCard}>
              <ActivityIndicator size="small" color={C.o2} />
              <Text style={styles.progressCardText}>{submitStep}</Text>
            </View>
          )}
          ...
          >
            <LinearGradient
              colors={(!isFormValid || isSubmitting) ? [C.faint, C.faint] : [C.o2, C.o]}
              start={{ x: 0, y: 0 }}
              end={{ x: 0, y: 1 }}
              style={[styles.submitButton, (!isFormValid || isSubmitting) && styles.buttonDisabled]}
            >
              <Text style={[styles.submitButtonText, (!isFormValid || isSubmitting) && styles.submitButtonTextDisabled]}>
                {isSubmitting ? 'Processing...' : '🚀 Complete Onboarding'}
              </Text>
            </LinearGradient>
          </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  // ── heading ──
  questionSection: { alignItems: 'center', marginTop: 22, marginBottom: 22, paddingHorizontal: 22 },
  ...
  // uat fill
  uatFillBtn: {
    backgroundColor: 'rgba(247,200,90,0.1)', borderWidth: 1.5, borderColor: 'rgba(247,200,90,0.35)',
    borderRadius: 16, padding: 14, alignItems: 'center', marginHorizontal: 20, marginBottom: 20,
  },
  uatFillBtnText: { color: C.gold, fontSize: 13.5, fontWeight: '700' },
  uatFillSubText: { color: 'rgba(247,200,90,0.7)', fontSize: 11, marginTop: 3 },
  ...
  genderBtn: {
    flex: 1, paddingVertical: 12, borderRadius: 13, borderWidth: 1.5, borderColor: C.line2,
    alignItems: 'center', backgroundColor: C.surface,
  },
  genderBtnActive: { backgroundColor: 'rgba(255,106,26,0.14)', borderColor: C.o },
  genderBtnText: { color: C.muted, fontSize: 13.5, fontWeight: '600' },
  genderBtnTextActive: { color: C.o2 },

  chipRow: { flexDirection: 'row', gap: 8, paddingBottom: 4 },
  chip: {
    paddingVertical: 8, paddingHorizontal: 14, borderRadius: 30, borderWidth: 1.5,
    borderColor: C.line2, backgroundColor: C.surface,
  },
  chipActive: { backgroundColor: 'rgba(255,106,26,0.14)', borderColor: C.o },
  chipText: { color: C.muted, fontSize: 13, fontWeight: '600' },
  chipTextActive: { color: C.o2 },

  divider: { height: 1, backgroundColor: C.line, marginVertical: 8, marginBottom: 20 },
  sectionLabel: { fontSize: 11.5, fontWeight: '700', color: C.muted, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 20 },
  ...
  testCardText: { color: C.muted, fontSize: 12.5, lineHeight: 20 },

  errorCard: {
    backgroundColor: 'rgba(255,107,107,0.08)', borderWidth: 1.5, borderColor: 'rgba(255,107,107,0.3)',
    borderRadius: 16, padding: 14, marginBottom: 16,
  },
  errorCardTitle: { color: C.bad, fontSize: 13.5, fontWeight: '700', marginBottom: 6 },
  errorCardText: { color: '#ffb3b3', fontSize: 12.5, lineHeight: 19 },

  progressCard: {
    flexDirection: 'row', alignItems: 'center', gap: 12, backgroundColor: 'rgba(255,106,26,0.08)',
    borderRadius: 14, padding: 14, marginBottom: 12,
  },
  progressCardText: { color: C.ink, fontSize: 13.5, fontWeight: '600' },

  buttonSection: { padding: 20, gap: 12, marginBottom: 20 },
  submitButtonWrap: { borderRadius: 15, shadowColor: C.o, shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.4, shadowRadius: 16, elevation: 6 },
  submitButton: { paddingVertical: 18, borderRadius: 15, alignItems: 'center' },
  buttonDisabled: { shadowOpacity: 0 },
  submitButtonText: { color: '#1a0d04', fontSize: 15.5, fontWeight: '700' },
  submitButtonTextDisabled: { color: C.muted },
  devButton: {
  ...
  // ── Modal styles ──
  modalOverlay: { flex: 1, backgroundColor: 'rgba(8,6,10,0.8)', justifyContent: 'flex-end' },
  modalSheet: {
  ...
  modalHandle: { width: 40, height: 4, backgroundColor: C.faint, borderRadius: 2, marginBottom: 24 },
  modalEmoji: { fontSize: 44, marginBottom: 12 },
  modalTitle: { fontSize: 22, fontWeight: '700', color: C.ink, marginBottom: 8, textAlign: 'center' },
  modalSubtitle: { fontSize: 14, color: C.muted, lineHeight: 21, textAlign: 'center', marginBottom: 24 },
  modalSteps: { width: '100%', backgroundColor: C.surface, borderRadius: 16, padding: 16, marginBottom: 22, gap: 12, borderWidth: 1, borderColor: C.line },
  modalStep: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  modalStepDone: { fontSize: 16 },
  modalStepPending: { fontSize: 16 },
  modalStepText: { fontSize: 13.5, color: C.muted, fontWeight: '500' },
  modalAuthBtnWrap: { width: '100%', borderRadius: 14, marginBottom: 12, shadowColor: C.o, shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.35, shadowRadius: 14, elevation: 5 },
  modalAuthBtn: { width: '100%', paddingVertical: 16, borderRadius: 14, alignItems: 'center' },
  modalAuthBtnText: { color: '#1a0d04', fontSize: 14.5, fontWeight: '700' },
  modalNoLink: {
    width: '100%', backgroundColor: 'rgba(247,200,90,0.08)', borderWidth: 1.5, borderColor: 'rgba(247,200,90,0.3)',
    borderRadius: 14, padding: 14, marginBottom: 12,
  },
  modalNoLinkText: { color: C.gold, fontSize: 12.5, lineHeight: 19, textAlign: 'center' },
  modalNote: { fontSize: 12, color: C.muted, textAlign: 'center', marginBottom: 20, lineHeight: 18 },
  modalDoneBtn: {
    width: '100%', backgroundColor: C.good, paddingVertical: 14, borderRadius: 14,
  ...
  authStatusError: { backgroundColor: 'rgba(255,107,107,0.1)', borderColor: 'rgba(255,107,107,0.35)' },
  authStatusText: { fontSize: 12.5, color: C.ink, textAlign: 'center', fontWeight: '500' },
  modalDismissBtn: { paddingVertical: 8 },
  modalDismissBtnText: { color: C.faint, fontSize: 13 },

  // Success state (kept for parity — unused visual states preserved)
  successContainer: { flex: 1, padding: 24, paddingTop: 80, alignItems: 'center', gap: 16 },
  successEmoji: { fontSize: 64, marginBottom: 8 },
  successTitle: { fontSize: 32, fontWeight: '700', color: C.ink, textAlign: 'center' },
  successSubtitle: { fontSize: 16, color: C.muted, lineHeight: 26, textAlign: 'center' },
  successSteps: { width: '100%', backgroundColor: C.surface, borderRadius: 16, padding: 20, gap: 12, borderWidth: 1, borderColor: C.line },
  successStep: { fontSize: 15, color: C.ink, fontWeight: '500' },
  authButton: { width: '100%', backgroundColor: C.o, paddingVertical: 18, borderRadius: 15, alignItems: 'center', marginTop: 8 },
  authButtonText: { color: '#1a0d04', fontSize: 16, fontWeight: '700' },
  authNote: { fontSize: 13, color: C.muted, textAlign: 'center', lineHeight: 20 },
  doneButton: { width: '100%', paddingVertical: 14, borderRadius: 15, alignItems: 'center', borderWidth: 1.5, borderColor: C.line2 },
  doneButtonText: { color: C.ink, fontSize: 15, fontWeight: '500' },
});
