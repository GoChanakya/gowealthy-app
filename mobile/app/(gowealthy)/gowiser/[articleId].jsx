...
//     //   }
//     // }
//     if (articleData.mcqHtml) {
//   const mcqs = extractMCQs(articleData.mcqHtml);
//   if (mcqs.length > 0) {
//     slidesList.push({
//       type: 'mcq',
//       content: mcqs
...
//   const extractMCQs = (mcqHtml) => {
//   const mcqs = [];
//   const questionRegex = /(\d+\.\s.*?)<\/p>([\s\S]*?)(?=<div|$)/gi;
//   const parser = mcqHtml.split('<div style=');
  
//   parser.forEach((block) => {
//     const questionMatch = block.match(/<p[^>]*>(.*?)<\/p>/i);
//     const buttonMatches = [...block.matchAll(/onclick="checkAnswer\(this,\s*(true|false)/gi)];
//     const buttonTexts = [...block.matchAll(/<button[^>]*>([^<]+)<\/button>/gi)];
    
//     if (questionMatch && buttonTexts.length > 0) {
//       mcqs.push({
//         question: questionMatch[1].replace(/<[^>]*>/g, ''),
//         options: buttonTexts.map((btn, i) => ({
//           text: btn[1].trim(),
//           isCorrect: buttonMatches[i]?.includes('true') || false
//         }))
//       });
//     }
//   });
...
//   if (scrolling || mcqTapped.current) return;
...
//   const handleMCQAnswer = (qi, oi, isCorrect) => {
//   if (mcqAnswers[qi] !== undefined) return;
//   mcqTapped.current = true;
//   setMcqAnswers(prev => ({ ...prev, [qi]: { selected: oi, isCorrect } }));
//   setTimeout(() => { mcqTapped.current = false; }, 300);
// };
...
//         onScrollEndDrag={() => setTimeout(() => setScrolling(false), 100)}
//       >
//         <Text style={styles.faqMainTitle}>Test Your Knowledge</Text>
//         <Text style={styles.faqSubtitle}>Answer all 4 questions</Text>

//         <View style={styles.faqContainer}>
//           {slide.content.map((mcq, qi) => (
//             <View key={qi} style={[styles.faqItem, { borderLeftColor: qi % 2 === 0 ? ORANGE : '#6C50C4' }]}>
//               <View style={{ padding: 16 }}>
//                 <Text style={styles.faqQuestion}>{mcq.question}</Text>
//                 <View style={{ gap: 8, marginTop: 12 }}>
//                   {mcq.options.map((opt, oi) => {
//                     const answered = mcqAnswers[qi] !== undefined;
//                     const isSelected = mcqAnswers[qi]?.selected === oi;
//                     const isCorrect = opt.isCorrect;

//                     let bgColor = colors.optionBackground;
//                     let borderColor = 'transparent';
//                     if (answered && isCorrect) { bgColor = 'rgba(39,174,96,0.2)'; borderColor = '#27ae60'; }
//                     else if (answered && isSelected && !isCorrect) { bgColor = 'rgba(231,76,60,0.2)'; borderColor = '#e74c3c'; }

//                     return (
//                       <TouchableOpacity
//                         key={oi}
//                         onPress={() => handleMCQAnswer(qi, oi, isCorrect)}
//                         disabled={answered}
//                         style={{
...
//                         }}
//                       >
//                         <Text style={{ color: colors.textColor, fontSize: 14 }}>{opt.text}</Text>
//                       </TouchableOpacity>
//                     );
//                   })}
//                 </View>
//                 {mcqAnswers[qi] !== undefined && (
//                   <Text style={{ marginTop: 10, fontSize: 13, fontWeight: '600', color: mcqAnswers[qi].isCorrect ? '#27ae60' : '#e74c3c' }}>
//                     {mcqAnswers[qi].isCorrect ? '✓ Correct!' : '✗ Wrong! Correct answer highlighted above.'}
//                   </Text>
//                 )}
//               </View>
//             </View>
...
const ArticleStoryView = () => {
  ...
  const fetchArticle = async () => {
    try {
      ...
      if (phone) {
        const blogReadRef = doc(db, 'questionnaire_submissions', phone, 'blogsRead', articleId);
        const blogReadSnap = await getDoc(blogReadRef);
        if (blogReadSnap.exists()) {
  ...
  const createSlides = (articleData) => {
    ...
    if (sections.length > 0) {
      sections.forEach((section) => {
        slidesList.push({ type: 'content', content: section });
      });
    } else {
      const bodyContent = cleanHtmlContent(articleData.body);
      const paragraphs = bodyContent.split('\n\n').filter(p => p.trim().length > 0);
    ...
    if (articleData.socialMediaPosts && articleData.socialMediaPosts.length > 0) {
      slidesList.push({ type: 'social', content: articleData.socialMediaPosts });
    }

    if (articleData.mcqHtml) {
      const mcqs = extractMCQs(articleData.mcqHtml);
      if (mcqs.length > 0) {
        slidesList.push({ type: 'mcq', content: mcqs });
      }
    }

    slidesList.push({
      type: 'end',
      content: { xp: articleData.xp, tags: articleData.tags || [] }
    });
  ...
  const handleTouchEnd = (event) => {
    ...
    if (locationX < leftZone && currentSlide > 0) {
      ...
      console.log('nextSlide:', nextSlide, 'slides.length:', slides.length, 'hasAwardedXP:', hasAwardedXP, 'alreadyEarned:', alreadyEarned);
      if (nextSlide === slides.length - 1 && !hasAwardedXP && !alreadyEarned) {
        awardXP();
      }
    }
  };
    ...
    try {
      ...
      if (phone && article) {
        ...
        const ledgerDoc = await addDoc(ledgerRef, {
          ...
          balanceAfter: currentBalance + xpAmount,
          source: 'blog_read',
          sourceId: articleId,
          createdAt: serverTimestamp()
        });

        await setDoc(doc(db, 'questionnaire_submissions', phone, 'blogsRead', articleId), {
        ...
        await updateDoc(userRef, {
          'xp.balance': increment(xpAmount),
          'xp.totalEarned': increment(xpAmount),
                        ...
                        {mcq.options.map((opt, oi) => {
                          ...
                          return (
                            <TouchableOpacity
                              ...
                              onPress={() => handleMCQAnswer(qi, oi, isCorrect)}
                              disabled={answered}
                              style={{ backgroundColor: bgColor, borderWidth: 1, borderColor, padding: 12, borderRadius: 10 }}
                            >
                              <Text style={{ color: colors.textColor, fontSize: 14 }}>{opt.text}</Text>
                            </TouchableOpacity>
                          );
        ...
        return (
          <View style={styles.slideContainer}>
            <View style={styles.endContent}>
              ...
              {alreadyEarned ? (
                <View style={styles.alreadyEarnedBadge}>
                  <Text style={styles.alreadyEarnedText}>✓ XP Already Earned</Text>
                </View>
              ) : (
                <Text style={styles.endSubtitle}>You've earned {slide.content.xp} XP</Text>
              )}
      ...
      >
        ...
        {showXPAnimation && (
          <Animated.View
            style={[
              styles.xpAnimationContainer,
              { transform: [{ scale: xpScale }], opacity: xpScale }
            ]}
          >
            <View style={styles.xpAnimationBadge}>
              <Text style={styles.xpAnimationEmoji}>🎉</Text>
